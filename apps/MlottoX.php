...
class MlottoX extends UssdApplication {


    public function getResponseX($body) {
        $this->initValues($body);

        return $this->result("end", "Welcome to MLOTTO\nThis service is under construction");
    }
    ...
    public function getResponse($body) {
        ...
        $post = Body::Json($data);
        $response = Unirest::post("https://ussd.mlotto.net/novaji/", $headers, $post);
