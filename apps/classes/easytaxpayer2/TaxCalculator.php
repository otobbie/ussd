 ...
 * @author Chuks
 */
class TaxCalculator extends Easytax2 {
	...
	public function calculateSelfAssessment($arr) {
		switch ($arr['currentStep']) {
			case 5:
				return $this->result("continue", $this->render('self_assessment') . "\n" . $this->render('monthly_income'));
			case 6:
				$amount = is_numeric($arr['userInputs'][5]) ? $arr['userInputs'][5] : 0.00;
				// Calculate tax
				$this->payable = $amount > 0 ? $amount * ($this->mintaxrate / 100) : $this->tax;

				return $this->result('end', $this->render('self_assessment') . "\n"
						. $this->render('lbl_monthly_income') . number_format($amount > 0 ? $amount : 0.00, 2) . "\n"
						. $this->render('lbl_min_tax') . number_format($this->payable, 2) . "\n"
						. $this->render('lbl_min_tax_rate') . number_format($this->mintaxrate, 2) . "%");
		}
	}
	...
	public function calculatePayee($arr) {
		...
		switch ($arr['currentStep']) {
			case 5:
				return $this->result('continue', $heading . $this->render('monthly_basic'));
			case 6:
				$paye = $this->computePaye($arr['userInputs'][5]);
				return $this->result('end', $this->render('lbl_estimated') . $heading . $this->render('lbl_gross') . number_format($paye['gross'], 2) .
